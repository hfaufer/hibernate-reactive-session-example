...
/**
 * Demonstrates the use of Hibernate Reactive with the {@link io.smallrye.mutiny.Uni Mutiny}-based API.
 */
    ...
    void main(final String[] args) {
        ...
        try (SessionFactory factory = createEntityManagerFactory(persistenceUnitName(args))
            ...
            factory.withTransaction(
                            ...
                            (session, _) -> session.persistAll(author1, author2)
                    )
            ...
            factory.withSession(
                            ...
                            session -> session.find(Book.class, book1.getId())
                                    // Print its title.
                                    .invoke(book -> IO.println(book.getTitle() + " is a great book!"))
                    )
                    .await().indefinitely();
            ...
            factory.withSession(
                            ...
                            session -> session.find(Author.class, author1.getId(), author2.getId())
                                    .invoke(authors -> authors.forEach(author -> IO.println(author.getName())))
                    )
                    .await().indefinitely();
            ...
            factory.withSession(
                            ...
                            session -> session.createQuery(
                                    ...
                                    .getResultList()
                                    .invoke(rows -> rows.forEach(row -> out.printf("%s (%s)\n", row[0], row[1]))))
                    .await().indefinitely();
            ...
            factory.withSession(
                            ...
                            session -> {
                                ...
                                return session.createQuery(query).getResultList().invoke(
                                        books -> books.forEach(book -> IO.println(book.getTitle()))
                                );
                            }
                    )
                    .await().indefinitely();
            ...
            factory.withSession(
                            ...
                            session -> session.find(Book.class, book1.getId())
                                    ...
                                    .call(book -> session.fetch(book, Book_.coverImage)
                                            ...
                                            .invoke(coverImage -> IO.println(new String(coverImage)))
                                    )
                    )
                    .await().indefinitely();
